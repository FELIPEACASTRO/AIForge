...
- **"Fade the public" / reverse line movement (RLM)** — the idea that when tickets pile on one side while the *line* moves the other way, sharp money is on the other side. Soccer split data:
  - [**Action Network — soccer public betting**](https://www.actionnetwork.com/soccer/public-betting): percentage of **bets (tickets)** vs **money (dollars)** with a **DIFF** column — the standard bets-vs-money split view. Coverage is limited to fixtures carried by US sportsbooks, and pages are empty when no such market is live.
