...
Prediction markets convert crowd money into **real-time probabilities** for discrete events (elections, rate decisions, CPI prints, earnings, geopolitics). Treated as a live macro signal, the marginal price is a calibrated forecast of `P(event)`. In 2025 the asset class went institutional: **Intercontinental Exchange (parent of NYSE) committed up to US$2bn to Polymarket** at a ~US$8bn pre-money / ~US$9bn post-money valuation, and ICE will **become a global distributor of Polymarket's event-driven data** — i.e., prediction-market odds are becoming a licensed market-data product alongside equities and futures ([ICE press release, Oct 2025](https://ir.theice.com/press/news-details/2025/ICE-Announces-Strategic-Investment-in-Polymarket/default.aspx); [Fortune, Oct 2025](https://fortune.com/crypto/2025/10/07/polymarket-2-billion-intercontinental-exchange-new-york-stock-exchange-9-billion/)). Rival **Kalshi** repriced fast: US$5bn (Oct 2025) → US$11bn (Dec 2025 Series E) → **US$22bn on a US$1bn Series F (May 2026)**, and by mid-2026 was reportedly raising at ~US$40bn ([Kalshi — US$22bn Series F](https://news.kalshi.com/p/kalshi-raises-1-billion-22-billion-valuation-institutional-demand-surges); [TechCrunch, Oct 2025](https://techcrunch.com/2025/10/10/kalshi-hits-5b-valuation-days-after-rival-polymarket-gets-2b-nyse-backing-at-8b/)).
...
| **Kalshi** | CFTC-regulated US event exchange; cash-settled binary contracts | **CFTC DCM** (regulated) | Yes (public read) | REST + WebSocket, base `https://api.elections.kalshi.com/trade-api/v2` (also `external-api.kalshi.com`); markets, candlesticks, order book; `kalshi-python` client | [docs.kalshi.com](https://docs.kalshi.com/) |
...
| **PredictIt** | US-politics real-money market (academic-exempt) | CFTC no-action since 2014; **amended July 2025** (operator moved to Prediction Market Research Consortium; old caps lifted) — still operating | Yes | Read-only `predictit.org/api/marketdata/all/` (no auth; ~1 req/min) | [predictit.org](https://www.predictit.org/) |
...
**Free aggregation / research tooling.** Beyond the per-venue APIs, two patterns help researchers: (a) unified feeds such as **SimpleFunctions** (`/api/changes` real-time cross-venue change feed, plus CLI/MCP) and the curated [`spfunctions/awesome-prediction-markets`](https://github.com/spfunctions/awesome-prediction-markets) and [`spfunctions/prediction-markets-reading`](https://github.com/spfunctions/prediction-markets-reading) lists; (b) joining odds to **free macro series** — FRED, World Bank, Eurostat (`ec.europa.eu/eurostat/api`), OECD SDMX, and DBnomics (`db.nomics.world`) — to build market-implied-vs-consensus spreads. Polymarket's full on-chain history is also queryable via **The Graph / Bitquery** subgraphs and **Dune** (see §6), which avoids rate limits on the hosted APIs.
...
| **Climate TRACE** | Asset-level emissions inventory, **744M+ assets / 2.7M+ sources** (power, steel, ships, refineries); monthly releases | **Free** | Open download + REST API | [climatetrace.org](https://climatetrace.org/) |
| **Climate Policy Radar** | Full text of **30,000+ climate laws/policies** + UN submissions, AI search | **Free** (open data, nonprofit) | Open-data dumps (Hugging Face `ClimatePolicyRadar/all-document-text-data` + `climatepolicyradar/open-data` repo); live API "coming soon" | [climatepolicyradar.org](https://www.climatepolicyradar.org/) |
...
| **Wikimedia Pageviews** | Attention proxy per article/project (since 2015), bot-filtered | **Free** | AQS REST Pageviews API (docs at `doc.wikimedia.org/analytics-api`); hourly dumps at `dumps.wikimedia.org`; Wikimedia Enterprise (paid) for bulk | [Wikimedia Analytics API](https://doc.wikimedia.org/analytics-api/) |
...
| **RavenPack / Bigdata.com** | Investment-grade news analytics & sentiment, 40,000+ sources, 300,000+ entities; new LLM platform **Bigdata.com** (launched Oct 2024) | Paid | REST/SFTP feeds; **WRDS** access for academics | [ravenpack.com](https://www.ravenpack.com/) |
...
| **Kpler (incl. MarineTraffic/FleetMon)** | Commodity flows + AIS vessel tracking (acquired MarineTraffic & FleetMon, 2023) | Paid (enterprise) | API; MarineTraffic now enterprise-only | [kpler.com](https://www.kpler.com/) |
...
| **Sensor Tower (acq. data.ai)** | App downloads, revenue, MAU/DAU; **data.ai acquired by Sensor Tower (2024)** | Paid | Store Intelligence API | [sensortower.com](https://sensortower.com/) |
...
- **Survivorship & look-ahead (viés de sobrevivência / antecipação)**: include delisted/dead entities and weight by survival; in hedge-fund data, look-ahead bias alone can reach **~3.8% at a four-quarter horizon** (Baquero, ter Horst & Verbeek, *Survival, Look-Ahead Bias, and Persistence in Hedge Fund Performance*) ([JFQA 2005](https://www.cambridge.org/core/journals/journal-of-financial-and-quantitative-analysis/article/abs/survival-lookahead-bias-and-persistence-in-hedge-fund-performance/309BF3E4C6A6EF54604E325F234ED3A5)). Magnitudes are larger for higher-risk (small/levered) cross-sections.
