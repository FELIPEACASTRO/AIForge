...
| *Deep Learning for Options Trading: An End-To-End Approach* | arXiv paper | Data-driven trading rules over option chains | [arxiv.org/html/2407.21791v1](https://arxiv.org/html/2407.21791v1) |
| *Neural networks for option pricing and hedging* | arXiv paper | NN pricing/hedging benchmarks | [arxiv.org/pdf/1911.05620](https://arxiv.org/pdf/1911.05620) |
...
| **GitHub** | Open-source code | `vollib`/`py_vollib` (Black-Scholes/Greeks), `mibian`, `OptionSuite` backtester | [py_vollib](https://github.com/vollib/py_vollib) · [mibian](https://github.com/yassinemaaroufi/MibianLib) |
...
B3 options use Brazilian naming — *trava de alta/baixa* (vertical spreads), *borboleta* (butterfly), *condor*, *financiamento* (covered call), *compra/venda de volatilidade* (straddle/strangle) — and distinct quirks: on B3 equity options, **virtually all puts are European-style while most calls are American-style** (so early-exercise/assignment risk lives mainly on short calls), liquidity concentrates around a few underlyings (**PETR/VALE/BOVA11/index**), and expiration/settlement calendars differ from the US. Always confirm the series specs on B3.
...
> **B3 reality check.** Liquidity outside the top names and front-month strikes can be thin — wide bid/ask spreads quietly eat your edge. American-style early assignment (*exercício antecipado*) is a real risk on short ITM calls before dividends/JCP. Tax treatment (IR, *darf*, day-trade vs. swing) is Brazil-specific — consult current Receita Federal rules and a contador, not a YouTube video.
